...
const Header = () => {
  ...
  const pages = menuData.filter((item) => item.path);

  // Find current page index in pages
  const currentIndex = pages.findIndex((page) => page.path === usePathName);

  // Find next page or loop to first
  const nextPage =
    ...
    <>
      <header
        className={`header top-0 left-0 z-40 flex w-full items-center ${
          sticky
            ? "dark:bg-dark/800 dark:shadow-sticky-dark shadow-sticky fixed z-9999 bg-white/80 backdrop-blur-xs transition"
